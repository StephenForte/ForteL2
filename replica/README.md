...
| Sync check | Public replica RPC: `https://fortel2-replica-rpc.onrender.com`. `./scripts/replica-sync-check.sh` if you have a reachable URL. The live verifier is still a **Private Service** — do not convert it to Web. Operator inspection of the disk: dashboard **Web Shell** (`localhost:10000` EL / `:9545` op-node). |
...
| SettlementOS | **Read** RPC over Render private network (`http://fortel2-replica:10000`). Public replica for others: `https://fortel2-replica-rpc.onrender.com`. Public sequencer tip: `https://fortel2-sequencer-rpc.onrender.com`. **Writes** use Cloudflare Access (hostname unpublished in `rail-interface.json`, D-0035). | Only if redeploy changed genesis |
| settlementos-explorer | Indexed / browser reads — sequencer-tip URL first, replica URL as overnight fallback | Same as SOS |
...
After SOS-heavy demos, if Mac and replica tips diverge, run `FORTEL2_ENV=.env.sepolia REPLICA_L2_RPC_URL=https://fortel2-replica-rpc.onrender.com ./scripts/replica-sync-check.sh`. Private-service disk inspection is still Web Shell per D-0016.

...
func TestIsHardTxFailure(t *testing.T) {
	...
	cases := []struct {
		...
		// Reorg receipt flicker must stay soft — clearing would resubmit under a new nonce.
...
func TestShouldClearPending(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "reorg receipt loss but tx still known keeps",
			waitErr: errors.New("receipt for 0xabc disappeared after reorg"),
			l1: &fakeL1TxTracker{
				byHashTx:      dummyTx,
				byHashPending: true,
			},
		...
		{
			name:    "reorg then receipt returns keeps",
			waitErr: errors.New("receipt for 0xabc disappeared after reorg"),
			l1: &fakeL1TxTracker{

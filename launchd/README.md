...
`run_dev_wake.sh`, `run_dev_sleep.sh`, and `refresh_health.sh` prefix every line they print with local ISO-8601 (`YYYY-MM-DDThh:mm:ss±hhmm`) and merge stderr into that stream so a reader can date a line without `launchctl print` (D-0138). Editing wrapper **contents** does not change the fdautil allowlist (that authorises the command string); do not rename or move them. Merge is not deploy — `./scripts/deploy-agents.sh` then the next scheduled run (D-0113 Finding 2 / D-0134).

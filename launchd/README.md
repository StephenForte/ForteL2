...
Sleep/wake stop and start the **Mac** stack only. They do **not** Suspend Render or pause QuickNode — do those in the dashboards when you want zero credit burn while remote. They also do **not** stop `com.steve.fortel2-cloudflared`: the tunnel stays up while the write-filter origin goes dark for 23:45–03:00 (D-0034).
...
# Write-RPC tunnel — only after config/cloudflared-write.yml has no REPLACE_WITH_ placeholders.

...
**Scheduled on the Mac mini (launchd):** checked-in agents run Sepolia sleep at **23:00** and wake at **08:00** local (`launchd/com.steve.fortel2-sleep.plist`, `…-wake.plist`). Install once per the steps in `launchd/README.md` (replace any old `crontab` entries so jobs do not double-fire). User LaunchAgents require a logged-in session on the mini. Render Suspend / QuickNode pause remain manual dashboard steps when you are remote.

...
**Automated top-ups (external to this repo).** Since 2026-08-11 the batcher is topped up by operator-run automation hosted outside this tree, maintaining a **~0.6 ETH minimum** (operator policy). Two consequences for anyone reading the tooling: (1) `0.15` is the *tooling* floor — the level at which batching actually breaks — while `0.6` is the *funding* policy that triggers a top-up, so `days_to_floor` measures time-to-breakage, not time-to-refill; (2) `gas-runway.sh` deliberately skips intervals where the balance rose, so every automated top-up erases a burn-measurement window. For a clean burn number (e.g. for the P7-0 cost model) let the balance draw down across at least one uninterrupted hour, or have the funding job log its transfers so they can be subtracted rather than skipped. **The funding service is not identified in this repo** — see `tasks/hardening-findings.md` § "Batcher funding automation".

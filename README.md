...
8b. **Register the fault-proof game (D-0061)** — only once the network is healthy, and deliberately *outside* the outage window. Commit the new `rollup.json` to `deployments/sepolia/`, run the CI workflow to build a Kona prestate with that config baked in, verify it locally with `cannon witness --input <file>`, then set `FAULT_GAME_ABSOLUTE_PRESTATE` in `.env.sepolia` to that commitment and run `FORTEL2_ENV=.env.sepolia ./scripts/02-deploy-contracts-sepolia.sh` **without** `FORCE_SEPOLIA_REDEPLOY`. That second apply registers the type-8 game with `MakeRespected`. Nothing here is time-pressured; the chain is up and writers are running.

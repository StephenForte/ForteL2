...
`refresh_health.sh` writes the same schema to `data/pipeline-health.json`. The `batcher` object keeps `post_count`, `last_hash`, `last_age_sec`, and `cadence_sec` with their original types and null semantics, and adds `verdict`: **`healthy`** when the L1 scan (default **60** Sepolia blocks, ~12 min / ≥2 batch cycles at the measured 360 s cadence) found at least one inbox post, or **`no-posts`** when that window is empty. Downstream consumers should read `verdict` rather than inventing thresholds from the raw counts (D-0141).

...
1b. **Prestate gate — blocking, do not skip (D-0056).** op-deployer 0.7.1's built-in `DisputeAbsolutePrestate` default is `0x038512e0…6764d54c`, which the superchain registry lists as a **cannon32** artifact, while the deployed MIPS64 VM reports `stateVersion() = 8` and the pinned `cannon` embeds only that one VM. Left alone, the redeploy re-commits that stale default and the challenger cannot step a game on the new chain — a second network-wide wipe to fix. This has to happen **before you announce** (step 2), not after: it is artifact research plus a reviewed code change, and D-0049 says notice only goes out once all Phase 7 work is done. Slotting it after the writers are stopped would leave chain 852 down for the entire duration. (a) confirm which prestate release/type is stateVersion 8, (b) obtain that artifact and verify its commitment with `cannon witness --input <file>`, (c) land **F7-6** so `02-deploy-contracts-sepolia.sh` writes `faultGameAbsolutePrestate` into `[globalDeployOverrides]` and refuses to run without it. **Editing `deployments/sepolia/.deployer/intent.toml` by hand does not work** — the script rewrites that file from the environment on every run, before `op-deployer apply` reads it.

...
`blocks/config.js` and `blocks/.csp-header` are **gitignored** (regenerated by `gen-blocks-config.sh`). RPC failures surface as plain status text — views do not silently go stale. All chain-derived strings render via `textContent`.

...
8b. **Register the fault-proof game (D-0061)** — only once the network is healthy, and deliberately *outside* the outage window. Commit the new `rollup.json` to `deployments/sepolia/`, run the CI workflow to build a Kona prestate with that config baked in, verify it locally with `cannon witness --input <file>`, then run a **second** `op-deployer apply` that registers the type-8 game with that prestate and `MakeRespected`. Nothing here is time-pressured; the chain is up and writers are running.

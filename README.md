...
**Scheduled on the Mac mini (launchd):** checked-in agents run health at **05:00**, Sepolia sleep at **23:45**, and wake at **03:00** local (`launchd/com.steve.fortel2-{health,sleep,wake}.plist`). They execute a pinned clone of `main` at `/Users/steveforte/fortel2-agents` (not the mutable `~/ForteL2` checkout — D-0113 Finding 2). `lib.sh` derives `FORTEL2_ROOT` from that tree (an env `FORTEL2_ROOT=` is ignored with a warning). Create or fast-forward that clone with `./scripts/deploy-agents.sh` before copying plists; see `launchd/README.md`. The write tunnel is a dashboard-managed system LaunchDaemon (not a user LaunchAgent) — see [Authenticated Cloudflare tunnel](#authenticated-cloudflare-tunnel-t5-step-3--d-0034). Install once per the steps in `launchd/README.md` (replace any old `crontab` entries so jobs do not double-fire). User LaunchAgents require a logged-in session on the mini. Render Suspend / QuickNode pause remain manual dashboard steps when you are remote. The 03:00 wake retries op-proposer init (`PROPOSER_START_GRACE_SEC=15` × `PROPOSER_START_ATTEMPTS=3`) so a QuickNode 429 during the resolve-games collision no longer leaves a partial stack.

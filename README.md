...
1. **Announce** the reset to all replica operators (Render + Phase 3b friends) **and to SettlementOS** before touching anything — SOS needs at least a day's notice, and the new contract addresses once step 4 has produced them. A re-genesis expires every ForteL2 address they hold and breaks the ForteL2 rows in their live public explorer address book; forewarned it is one scheduled recovery cycle, discovered from a failing read it is an incident. Where the schedule allows, land this in the same beat as publishing the off-box write URL (`tasks/spike-t5-write-path.md` step 5) — separately, SOS pays that cycle twice.
...
3. **Choose Phase 7 immutables in `.env.sepolia` first** — edit `PROOF_MATURITY_DELAY_SECONDS`, `DISPUTE_GAME_FINALITY_DELAY_SECONDS`, `FAULT_GAME_CLOCK_EXTENSION`, `FAULT_GAME_MAX_CLOCK_DURATION`, and `FAULT_GAME_WITHDRAWAL_DELAY` directly in the file (sized minutes-to-hours for a realistic dispute game). Do **not** pass these as inline env overrides on the command line: `scripts/lib.sh` sources `.env.sepolia` *after* the process starts, so the file's values win and silently overwrite anything set inline for these five vars.

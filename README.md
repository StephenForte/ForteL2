...
1b. **Prestate + game-type gate — blocking, do not skip (D-0056 / D-0060).** Two things, not one. (a) op-challenger binds the fault-proof *program* to the game type at compile time: types **0** and **1** run `op-program`, which is **absent** from the pinned monorepo, so they can never play a game no matter what prestate they hold. Only **type 8** (`cannon-kona`) is playable here. (b) op-deployer's built-in `DisputeAbsolutePrestate` default is a **cannon32** artifact while the MIPS64 VM requires `stateVersion` 8. So before you announce: build a Kona prestate for chain 852 in CI with its rollup config baked in, verify it with `cannon witness --input <file>`, get `kona-host` onto the mini, and land the deploy-script change that registers an additional dispute game at **type 8** with that prestate and `MakeRespected`. **Editing `deployments/sepolia/.deployer/intent.toml` by hand does not work** — the script rewrites that file from the environment on every run, before `op-deployer apply` reads it.

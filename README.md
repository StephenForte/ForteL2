...
| **EL** | **op-geth → op-reth** (parallel; not a phase number) | **Tasks 1–7 and 9 executed; Task 8 engineering landed, clean-room deploy unrun.** Cutover 2026-09-02, promoted 2026-09-08 (D-0122) (block 473032, [D-0120](tasks/decisions.md)); [`tasks/prd-op-reth-migration.md`](tasks/prd-op-reth-migration.md); Task 7 Render replica: **DONE — public read + SOS private read serve op-reth since 2026-09-11 (D-0128)**; **Task 9 executed 2026-09-14 (fortel2-replica R-0019):** geth pserv + 50 GB disk and staging gateway deleted; public read re-verified `reth/v2.3.0-9384bc5` (the tip recorded alongside it, 982723, was a **stalled** head — the replica was down 12.7 h on a corrupt PublicNode L1 header; see the PRD § Task 9). **Friend-sync gate skipped, not met.** **No geth rollback resource.** **R-0017 disk follow-up CLOSED** (reth disk 10 → 50 GB; Render has no 25 GB size). |
...
4. **Writes** (tx submit, contract deploy) → D1 filter / Access hostname for remote SOS; operator tooling stays on loopback `:9545`. **Reads:** public replica `https://fortel2-replica-rpc.onrender.com` (~3 min L1 lag); public sequencer tip `https://fortel2-sequencer-rpc.onrender.com` (tip-follow; down 23:45–00:15). SOS on Render still uses private `http://fortel2-replica:10000` (D-0032). The Access write hostname stays **unpublished** in `rail-interface.json` (D-0035).
...
# tasks/task4-op-reth-faultproof.md is STATUS complete — not a Task 5 go
...
`refresh_health.sh` writes the same schema to `data/pipeline-health.json`. The `batcher` object keeps `post_count`, `last_hash`, `last_age_sec`, and `cadence_sec` with their original types and null semantics, and adds `verdict`: **`healthy`** when the L1 scan (default **60** Sepolia blocks, ~12 min / ≥2 batch cycles at the measured 360 s cadence) found at least one inbox post, or **`no-posts`** when that window is empty. Downstream consumers should read `verdict` rather than inventing thresholds from the raw counts (D-0141). The `proposer` object keeps `factory`, `game_count`, and `latest` (with `index` / `game_type` / `timestamp` / `age_sec` / `proxy`) unchanged, and adds `interval_sec` (the configured `SEPOLIA_PROPOSER_INTERVAL`, parsed to seconds) and `verdict`: **`healthy`** when the latest game's age — excluding any time inside the nightly dev-sleep window (the installed launchd sleep and wake jobs; checked-in schedule 23:45–00:15 PT; an absent plist pair is an assumed default, not a measurement — `devSleep.source` in this JSON), however many such windows it spans — is within the configured interval plus a 2 h grace; **`overdue`** past that threshold; or **`unknown`** when `game_count` is 0, the latest game's details are unavailable, the configured interval could not be parsed, or the L1 read itself failed (timeout, JSON-RPC error, malformed `gameAtIndex` reply) — `unknown` is never collapsed into `healthy`, and a failed read never leaves the `proposer` object `null` either (D-0142, D-0143).
...
**Availability:** the sequencer (and therefore this filter’s upstream) is stopped nightly **23:45–00:15** `America/Los_Angeles` (D-0026). There is no uptime commitment.
...
Nightly sleep/wake stop the sequencer; the tunnel process stays up and the origin goes dark for **23:45–00:15** `America/Los_Angeles` (D-0026). That window is expected, not an outage of the tunnel daemon.
...
**Scheduled on the Mac mini (launchd):** checked-in agents run health at **05:00**, Sepolia sleep at **23:45**, and wake at **00:15** local (`launchd/com.steve.fortel2-{health,sleep,wake}.plist`). `alert-watch.sh` and `pipeline-snapshot.py` take that window from the installed jobs, so a later launchd edit does not need a code change; machines without those plists assume 23:45–00:15 and label it a default (D-0144). They execute a pinned clone of `main` at `/Users/steveforte/fortel2-agents` (not the mutable `~/ForteL2` checkout — D-0113 Finding 2). `lib.sh` derives `FORTEL2_ROOT` from that tree (an env `FORTEL2_ROOT=` is ignored with a warning). Create or fast-forward that clone with `./scripts/deploy-agents.sh` before copying plists; see `launchd/README.md`. The write tunnel is a dashboard-managed system LaunchDaemon (not a user LaunchAgent) — see [Authenticated Cloudflare tunnel](#authenticated-cloudflare-tunnel-t5-step-3--d-0034). Install once per the steps in `launchd/README.md` (replace any old `crontab` entries so jobs do not double-fire). User LaunchAgents require a logged-in session on the mini. Render Suspend / QuickNode pause remain manual dashboard steps when you are remote. The 00:15 wake retries op-proposer init (`PROPOSER_START_GRACE_SEC=15` × `PROPOSER_START_ATTEMPTS=3`) so a QuickNode 429 during the resolve-games collision no longer leaves a partial stack.

**Hourly bond recovery (D-0074):** `com.steve.fortel2-resolve-games` runs `scripts/resolve-games-sepolia.sh --execute` at minute **0** of every hour, including the 23:45–00:15 sleep window. The script talks to Sepolia L1 only (DisputeGameFactory + DelayedWETH); it does not need the local L2 stack. Dry-run remains the script default — only this agent passes `--execute`. Logs: `~/Library/Logs/fortel2-resolve-games.{out,err}.log`.

**Ops alerting:** `scripts/alert-watch.sh` (hourly at **:30**, `com.steve.fortel2-alerts`) watches `data/funding-health.json` for a **FAIL** verdict and for a health pipeline older than 26 h, watches the resolve-games agent for silence (≤ 2 hourly cycles, with grace after a Mac-sleep gap) or persistent nonzero exits, and watches the dashboard-managed `com.cloudflare.cloudflared` system LaunchDaemon when its plist is present — `cloudflared-failing` on missing/unparseable `launchctl print` or any `not running` state (KeepAlive will not restart a clean exit); `cloudflared-no-edge` when that job reports `running` but loopback `cloudflared_tunnel_ha_connections` is 0; `cloudflared-restart-storm` when the launchd `runs` counter grows by more than 1 between watcher cycles (a single legitimate restart is quiet); `cloudflared-metrics-unreachable` on two consecutive failed loopback metrics reads (a later success resets the streak). Metrics URL comes from the err-log line `Starting metrics server on 127.0.0.1:…/metrics` (default `127.0.0.1:20241`); the write origin `:9555` and `fortel2-write.ente.ltd` are not probed (origin is dark 23:45–00:15 by design). No overnight-sleep grace (KeepAlive 24/7). A host without the tunnel plist is quiet, not an alert. It also watches the public replica JSON-RPC (`https://fortel2-replica-rpc.onrender.com`, one `eth_getBlockByNumber(latest)` per run, never a metered L1 URL) for a head that is **losing ground** (`replica-losing-ground`: age growing by more than a 600 s noise floor on two consecutive successful probes; one grown delta is quiet; the floor is one batcher channel plus margin so the ~6 min L1-derived sawtooth is not "losing ground"; a probe whose growth interval overlaps 23:45–00:15 PT, or the first probe within one watcher cycle after the wake, does not advance the streak — Render stays up, the Mac SOURCE does not), for a head older than **3 h** (`replica-head-stale`, exclusive, the backstop when trend has no prior sample), and for two consecutive probe failures (`replica-unreachable`; a later success resets the streak). `replica-head-stale` / `replica-unreachable` do not take Mac-sleep last_check grace. It also runs its own hourly L1 read against the DisputeGameFactory (two `eth_call`s — `gameCount` + `gameAtIndex` — the first metered dependency in this watcher, disclosed in D-0142) for `proposer-overdue`: the latest game's age, excluding any time inside 23:45–00:15 PT however many such windows it spans (reusing `in_dev_sleep_window`, never a second copy of the boundary), exceeding the configured `SEPOLIA_PROPOSER_INTERVAL` (default 8h; the legacy `PROPOSER_INTERVAL` key is ignored) plus a 2 h grace, exclusive — exactly at the threshold is quiet, one second past fires. A failed or inconclusive read (no games yet, missing factory, garbage/timeout, or an unparseable interval) is quiet once; the same condition id only fires as "cannot verify" after two consecutive such reads, and a later successful read resets that streak. On Sepolia it also watches pidfiles (`stack-missing` / `stack-down`); those still fire on their existing triggers, and when op-reth itself is among the missing processes a current ExEx panic in the last 64 KiB of `op-reth.log` (after that process's last start banner) is quoted in the same alert so `Proofs storage not initialized` (run `op-reth proofs init`) and `Parent hash mismatch` (delete `$DATADIR/historical-proofs`) are not conflated. A historical panic is ignored — the log is never the detector. **OK / WARN / INSUFFICIENT do not alert.** Channels are a macOS notification banner and a Resend email; either fires if the other is missing or broken. Fill `RESEND_API_TOKEN`, `ALERT_EMAIL_FROM` (optional; default `onboarding@resend.dev`), and `ALERT_EMAIL_TO` in local `.env.sepolia` — not in the tracked example. Merge of this script into `main` does not load it on the Mac mini — launchd runs `/Users/steveforte/fortel2-agents`; `./scripts/deploy-agents.sh` is what updates that clone.

Install & shakeout (Mac mini; file copy alone does not load the job):

```bash
cp /Users/steveforte/fortel2-agents/launchd/com.steve.fortel2-alerts.plist ~/Library/LaunchAgents/
```
...
**Bond recovery (respected type):** `FORTEL2_ENV=.env.sepolia ./scripts/resolve-games-sepolia.sh` dry-runs the in-flight working set (from a persisted low-water mark under `$DATA_DIR`) and sends nothing. `--full-scan` ignores the mark and re-reads every factory game. `--execute` broadcasts currently-ready legs (`resolveClaim` / `resolve` / the two `claimCredit`s) using ADMIN for gas; the 0.08 ETH still returns to `PROPOSER_ADDRESS`. The script does not sleep — re-run after the 1800s finality delay and the 3600s DelayedWETH delay. Games are selected by comparing each game's type to the snapshot's `respected_game_type` (one `AnchorStateRegistry.respectedGameType()` read per run — not a hardcoded 1 or 8). Missing type information on a game or snapshot is skipped, never selected. `RESOLVE_GAMES_MAX_TXS_PER_RUN` (default **5**) caps transactions actually sent each run so a backlog cannot burst in one unattended hourly pass; the remainder is picked up next hour via the watermark. `--max-games N` still caps selected games on a manual run. See `tasks/hardening-findings.md` § R-14 / § R-16.
...
1. **Choose Phase 7 immutables in `.env.sepolia` first** (US-070) — edit `PROOF_MATURITY_DELAY_SECONDS`, `DISPUTE_GAME_FINALITY_DELAY_SECONDS`, `FAULT_GAME_CLOCK_EXTENSION`, `FAULT_GAME_MAX_CLOCK_DURATION`, `FAULT_GAME_WITHDRAWAL_DELAY`, and `PREIMAGE_ORACLE_CHALLENGE_PERIOD` directly in the file (sized minutes-to-hours for a realistic dispute game). Confirm `ADMIN_PRIVATE_KEY` still derives `ADMIN_ADDRESS` after that edit (F7-10; any case). **Confirmed values (D-0049, `tasks/spike-phase-7-immutables.md`):** `FAULT_GAME_CLOCK_EXTENSION=600`, `FAULT_GAME_MAX_CLOCK_DURATION=7200`, `PREIMAGE_ORACLE_CHALLENGE_PERIOD=3600`, `PROOF_MATURITY_DELAY_SECONDS=1800`, `DISPUTE_GAME_FINALITY_DELAY_SECONDS=1800`, `FAULT_GAME_WITHDRAWAL_DELAY=3600`. `PermissionedDisputeGame.initialize` requires `maxClockDuration >= max(2*clockExtension, clockExtension+preimageOracleChallengePeriod)` — leaving the preimage period at op-deployer’s `86400` while setting extension=`600` / max=`7200` cannot create a game (`InvalidClockExtension`). `02-deploy-contracts-sepolia.sh` refuses that combo before apply. Do **not** pass these as inline env overrides on the command line: `scripts/lib.sh` sources `.env.sepolia` *after* the process starts, so the file's values win and silently overwrite anything set inline for these six vars.
1b. **Phase 7 code gate — blocking, do not skip (D-0060 / D-0061).** Two facts drive this. (a) op-challenger binds the fault-proof *program* to the game type at compile time: types **0** and **1** run `op-program`, which is **absent** from the pinned monorepo, so they can never play a game whatever prestate they hold. Only **type 8** (`cannon-kona`) is playable here. (b) op-deployer's built-in prestate default is a **cannon32** artifact while the MIPS64 VM requires `stateVersion` 8. Before you announce, land and verify: the deploy script able to emit the `dangerousAdditionalDisputeGames` stanza, the CI prestate workflow, `kona-host` on the mini, the `ADMIN_PRIVATE_KEY` / `ADMIN_ADDRESS` pairing guard (F7-10), and the duplicate/absent-immutable guard (**F7-11**, D-0065). `02-deploy-contracts-sepolia.sh` refuses a duplicated assignment of any of the six immutables on every run (the last assignment would otherwise silently win), and refuses an absent or empty one when `FORCE_SEPOLIA_REDEPLOY=1` or `FAULT_GAME_ABSOLUTE_PRESTATE` is set. **The prestate itself is built later, at step 8b** — it commits to a rollup config that does not exist until after the redeploy. **Editing `deployments/sepolia/.deployer/intent.toml` by hand does not work** — the script rewrites that file from the environment on every run, before `op-deployer apply` reads it.
2. **Announce** the reset to all replica operators (Render + Phase 3b friends) **and to SettlementOS**, **only once all Phase 7 coding/config work is complete and reviewed** (D-0049). **Done for this reset: sent 2026-08-21 13:00 PDT / 20:00Z** to SettlementOS, SOS Explorer, L2Replica and ChainBank (D-0067); no Phase 3b friends are active. Once sent, wait **≥1 day** (operator policy: **≥24h**) before stopping writers — for this reset that gate is **2026-08-22T20:00Z**. Immediately before step 3, run **`scripts/phase7-preflight.sh`**; it must print `ALL CHECKS PASSED`. SOS needs the notice, and the new contract addresses once step 4 has produced them. A re-genesis expires every ForteL2 address they hold and breaks the ForteL2 rows in their explorer address book; forewarned it is one scheduled recovery cycle, discovered from a failing read it is an incident. **Expired does not mean empty.** Contract addresses derive from deployer + nonce, so a redeploy from the same deployer walks the same `CREATE` sequence: repeat it exactly and the old addresses come back holding the *same* contracts; shift it — a funding deposit, a changed deploy order — and they come back holding *different* ones. Either way an old address may be live, so revalidate code, type and decimals rather than assuming it is dead. In the 2026-08-22 reset a funding deposit consumed nonce 0 first, shifting every contract one slot along: two mock-token addresses landed on tokens with different decimals, which renders a plausible number instead of an error and is the one failure mode here that is silent (D-0069 Finding 2). The Access write path is already live and **unpublished** (D-0035). Do **not** hold the redeploy to coincide with publishing that hostname — SOS withdrew that request (D-0029); the two are independent.
3. ~~**Stop Mac writers**~~ **— DONE 2026-08-22 ≈21:05Z.** `FORTEL2_ENV=.env.sepolia ./scripts/stop-all-sepolia.sh` — the old batcher/proposer/op-node/op-geth must not keep posting against the deployment about to be replaced.
4. ~~**Redeploy**~~ **— DONE 2026-08-22 21:14:51Z; do NOT re-run, this is the destructive step.** `FORTEL2_ENV=.env.sepolia FORCE_SEPOLIA_REDEPLOY=1 ./scripts/02-deploy-contracts-sepolia.sh` — `FORCE_SEPOLIA_REDEPLOY` is not set anywhere in `.env.sepolia`, so it's safe to pass inline. The script pairs `ADMIN_PRIVATE_KEY` to `ADMIN_ADDRESS` first (F7-10, case-insensitive); a mismatch or empty key aborts before `require_min_balance_eth` and before `rm -rf "$DEPLOY_DIR"`. It then refuses a duplicated assignment of any of the six immutables, and an absent or empty one, on this wipe path (F7-11). Leave `FAULT_GAME_ABSOLUTE_PRESTATE` unset (commented out in `.env.sepolia.example`); the script refuses that variable during the wipe because any hash present then commits to the old rollup config (D-0061). This is the step that actually produces the new L1 contracts, genesis, and `rollup.json` — `pack-replica-artifacts.sh` only copies whatever is already in `$DEPLOY_DIR`, so packing before redeploying just republishes the old (pinned) artifacts. Send the new proxy addresses to SOS from `deployments/sepolia/deployments.json`.
5. ~~**Pack + publish**~~ **— DONE 2026-08-22, published at `bebddc3`.** `FORTEL2_ENV=.env.sepolia ./scripts/pack-replica-artifacts.sh`, then push the new genesis/rollup into [fortel2-replica](https://github.com/StephenForte/fortel2-replica).
6. ~~**All operators wipe**~~ **— DONE 2026-08-22 ≈21:40Z.** Mac `data-sepolia` (`FORTEL2_ENV=.env.sepolia ./scripts/reset-sepolia.sh`) and every replica `/data`. All sides, never one.
7. ~~**All restart**~~ **— DONE 2026-08-22 21:44Z.** against the new artifacts (Mac: `FORTEL2_ENV=.env.sepolia ./scripts/start-all-sepolia.sh`).
8. ~~**Cross-check block hashes**~~ **— DONE 2026-08-22 21:50Z, matched first try; v7 bumped in #115.** (Mac vs each replica, same block number) before declaring the network healthy. Then bump `rail-interface.json` to **v7** with the new `bridge.*` proxies (leave v6 until this point). SOS redeploys-or-adopts on their side.
8b. **Register the fault-proof game (D-0061 / D-0063)** — only once the network is healthy, and deliberately *outside* the outage window. Four parts, in order. **(1)** Commit the new `rollup.json` to `deployments/sepolia/`, then run the CI workflow to build a Kona prestate with that config baked in. **(2)** Verify it locally with `cannon witness --input <file>` — this is a **gate, not a formality**: registration is once-only both in op-deployer's state check *and* on-chain (`SetDisputeGameImpl` requires `gameImpls(8) == address(0)`), so a wrong hash cannot be fixed by re-running apply. Correcting one means a manual L1ProxyAdminOwner `setImplementation` or another network-wide wipe. **(3)** Set `FAULT_GAME_ABSOLUTE_PRESTATE` in `.env.sepolia` to that commitment and run `FORTEL2_ENV=.env.sepolia ./scripts/02-deploy-contracts-sepolia.sh` **without** `FORCE_SEPOLIA_REDEPLOY`. That second apply registers the type-8 game with `MakeRespected`; the signing key must be **both** L1ProxyAdminOwner and Guardian, because `MakeRespected` calls `setRespectedGameType` and that is Guardian-gated. **(4)** Set `PROPOSER_GAME_TYPE=8`, then **stop and start** the proposer — a plain re-run of the start script is a silent no-op, because `start_bg` returns 0 when the pid is still alive (`scripts/lib.sh:122-125`) and the stock branch does not stop a live proposer first. Use the § Phase 5 kill switch: `kill "$(cat "$DATA_DIR/pids/op-proposer.pid")" && rm -f "$DATA_DIR/pids/op-proposer.pid"`, then `FORTEL2_ENV=.env.sepolia ./scripts/06-start-proposer-sepolia.sh`, and confirm the startup line reports `game-type=8`. The apply flips the chain's respected game type from 1 to 8 — games created *before* the flip keep their respected flag and nothing in flight breaks, but a proposer left on type 1 keeps posting games that cannot prove a withdrawal and cannot advance the anchor. Nothing here is time-pressured; the chain is up and writers are running.
...
Full phase table is in [Roadmap](#roadmap) above; acceptance criteria live in `tasks/prd-l2-learning-chain.md`. Phases **0–6 done** (Sepolia + Render replica + custom batcher/proposer/derivation + block viewer). **3a** native Mac L1 stays deferred. Next learning work: Phase **7** fault proofs ([`tasks/prd-phase-7-fault-proofs.md`](tasks/prd-phase-7-fault-proofs.md)) via the redeploy gate, Phase **3b** friend replicas ([`replica/FRIENDS.md`](replica/FRIENDS.md); Render path in fortel2-replica `RUNNING.md`), or leftover op-reth follow-ups ([`tasks/prd-op-reth-migration.md`](tasks/prd-op-reth-migration.md); Tasks 1–7 and 9 executed — chain 852 sequences on op-reth since 2026-09-02; Task 8 clean-room deploy unrun; Task 9 friend-sync gate skipped; no geth rollback). Product track: expand/execute [`tasks/prd-mainnet-pilot.md`](tasks/prd-mainnet-pilot.md). Do **not** start Phase 2b+ until all of these are true:
...
**Safe-head database (prerequisite for judging games):** `op-challenger` verifies a root claim by asking op-node for the safe head at the game's L1 head (`optimism_safeHeadAtL1Block`), which answers only when op-node runs with a safe-head database. Set `OP_NODE_SAFEDB_PATH` in `.env.sepolia` (e.g. `$DATA_DIR/safedb` — derived data, wipe-safe) **before** starting the stack; without it the challenger starts cleanly but every judgment fails with `safe head database not enabled`, and it can only skip or resolve, never verify (D-0082). The start scripts are cold-start-only (`assert_l2_ports_free` requires the whole stack down), so enabling it on a running stack means a full `stop-all-sepolia.sh` / `start-all-sepolia.sh` cycle, not an op-node-only restart. Games created before safedb was enabled stay un-judgeable forever — their L1 head predates the recording — and log judge-errors until they leave the game window; that is noise, not a fault.

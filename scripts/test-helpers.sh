...
# a missing Operator sequence heading fails closed (not a pass); clean repo files pass.
...
P7_BAD_OUT="$(
  PHASE7_README="$P7_FIXTURE_DIR/README.md" \
    "$P7_CHECK" 2>&1
...
P7_ANCHOR_OUT="$(
  PHASE7_PRD="$P7_FIXTURE_DIR/prd.md" \
    "$P7_CHECK" 2>&1
...
P7_OK_OUT="$("$P7_CHECK" 2>&1)" && P7_OK_EC=0 || P7_OK_EC=$?

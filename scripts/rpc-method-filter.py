#!/usr/bin/env python3
"""Loopback JSON-RPC method filter for the Sepolia sequencer write path (T5-D1).

Listens on loopback, forwards only an explicit allowlist of eth/net/web3 methods
to the full op-geth HTTP RPC (default :9545). cloudflared (later) dials this
process — never op-geth directly.
...
Env:
  L2_RPC_FILTER_LISTEN     host:port (default 127.0.0.1:9555); host must be loopback
  L2_RPC_FILTER_UPSTREAM   http(s) URL of op-geth (default http://127.0.0.1:9545)
"""
...
# Explicit method allowlist — namespaces eth / net / web3 only, by full name.
...
class FilterState:
    def __init__(self) -> None:
        upstream = _env("L2_RPC_FILTER_UPSTREAM", "http://127.0.0.1:9545")
...
def main() -> int:
    ...
    listen = _env("L2_RPC_FILTER_LISTEN", "127.0.0.1:9555")
    if ":" not in listen:

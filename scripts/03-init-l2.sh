...
if [[ ! -f "$GENESIS" ]]; then
  echo "ERROR: missing $GENESIS — run scripts/02-deploy-contracts.sh first" >&2
  exit 1
fi
...
if [[ -d "$DATADIR/geth" ]]; then
  echo "op-geth datadir already initialized at $DATADIR (skipping). Use scripts/reset.sh to wipe."
  exit 0
fi

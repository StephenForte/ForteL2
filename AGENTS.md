...
- Prefer **small, reversible diffs**. Phases **0–6** are done (including `batcher/`, `proposer/`, `derivation/`, `blocks/`). Phase **3** Render replica is done. Do **not** execute a Sepolia redeploy, wipe replica genesis, or start Phase **3a** unless asked. Phase **7** spec is `tasks/prd-phase-7-fault-proofs.md` (learning; wipe is operator-owned; order is § Operator sequence; `rail-interface.json` stays v6 until post-wipe proxies). Phase **3b** friend runbook is `replica/FRIENDS.md` (recruiting is operator-owned). **op-reth migration** (`tasks/prd-op-reth-migration.md`): Tasks 1–7 and Task 9 executed. Chain 852 sequences on **op-reth** since 2026-09-02, promoted 2026-09-08 (D-0122; D-0120 cutover; `FORTEL2_EL=reth` in `.env.sepolia`). **Task 8 engineering landed** (R-0018, #53/#55/#56, #58 / R-0020); **clean-room deploy still unrun** — do not read Task 8 as verified. **Task 7 closed 2026-09-12 (D-0127–D-0129):** the Render replica is `fortel2-replica-reth` (op-reth archive from snapshot 811872, `L1_RPC_FORCE=public` + `L1_RPC_KIND=standard`); public read `fortel2-replica-rpc` and SettlementOS private read point at `http://fortel2-replica-reth:10000` by env value. **Task 9 executed 2026-09-14 (fortel2-replica R-0019):** geth EL files deleted from that repo; the operator deleted the geth pserv `fortel2-replica` (`srv-d9fsgi3rjlhs73ceh6tg`, 50 GB disk) and staging gateway `fortel2-replica-reth-rpc` (`srv-dagr42tbedkc73c5mp80`). Re-verified after: public read `reth/v2.3.0-9384bc5`, SettlementOS `/api/networks` 200 — but the tip recorded alongside (982723) was a **stalled** head: the Render replica was wedged 12.7 h on a corrupt PublicNode L1 header (11703708) and was recovered 2026-09-14 by pointing `L1_RPC_URL` at QuickNode with `L1_RPC_FORCE=metered`. **Do NOT return it to `public` (D-0136):** measured 2026-09-15, `public` derives at 0.35 L1 blocks/min against Sepolia's ~5/min, so the replica cannot recover from any restart on it. `metered` is the standing configuration at ~$0.19/day. A tip number is not a liveness check; record head *age*. **There is no geth rollback:** files return from git; the Render disk does not — recovery is a new disk and a full resync. Do not treat `rollback-to-geth-sepolia.sh` or `FORTEL2_EL=geth` as a supported recovery. **The friend-sync gate was skipped, not met:** Task 9 Instructions required at least one friend clean sync; none has happened; two candidates expected the week of 2026-09-21; the operator chose to proceed. **R-0017 disk follow-up CLOSED:** `fortel2-replica-reth-data` resized 10 → 50 GB the same day (Render offers fixed sizes; 25 is not one). Render private hostnames are immutable slugs — never rename Render services to move traffic (D-0128). Do **not** wipe or `debug_setHead` the live reth datadir, re-apply the fortel2-replica Blueprint, set `karst_time`, or create `fortel2-node` unless asked. D-0125 script fixes landed (#217, D-0130): sidecar pid names are `op-reth-verifier` / `op-reth-verifier-node`; never stop the sidecar by the live name. **Live-datadir wipe/start guard landed:** `wipe_reth_datadir` and `start-op-reth-verifier.sh` refuse `$DATA_DIR/l2/op-reth` when live op-reth evidence is present (alive `op-reth` / `op-reth-node` pidfile, or a listener on `:9545`/`:9546`/`:9547`/`:9551`); a sidecar on a reth-live host must set `FORTEL2_RETH_DATADIR=$DATA_DIR/l2/spike-op-reth`. **op-challenger is intentionally STOPPED since 2026-09-13 (D-0132)** — QuickNode quota exhaustion; it had been failing every cycle since 2026-08-24 and its scan cost grows with the cumulative game count. Do not restart it as routine recovery; resolution and bond claims are handled by the hourly `resolve-games-sepolia.sh` launchd job, not the challenger. Do **not** set `SEPOLIA_CHALLENGER_GAME_WINDOW` — shrinking it saves nothing (D-0111 Finding 3, D-0132). Remaining: friend recruiting (Phase 3b, operator-owned); learning-oracles keep-on-geth vs follow-up (not settled). MR-3/4/5 stay parked until SOS asks.
...
# Task 5 already ran (2026-09-02). These scripts remain in-tree as history.
# rollback-to-geth-sepolia.sh is not a supported recovery — the Render geth disk is gone.
...
# Mac mini schedule: launchd health 05:00 / sleep 23:45 / wake 00:15 — see launchd/README.md (not cron)
...
- Sidecar pid names are `op-reth-verifier` / `op-reth-verifier-node`; never stop the sidecar by the live name (`op-reth` / `op-reth-node`).
- **L1 provider preflight (D-0123).** Before naming ANY L1 RPC provider for a derivation catch-up (replica sync, verifier bootstrap, friend node), run `scripts/l1-provider-preflight.sh` and paste its output. The script runs the two capability calls against the candidate URL: `eth_getBlockByNumber` for the L1 genesis origin (11545587) and `eth_getBlockReceipts` for a block ≥3 weeks old — both must return data. Then compute cost = remaining L1 blocks × measured per-block price vs the plan cap and put the number in front of the operator. Measured: publicnode prunes old receipts (tip-following only); Alchemy free ≈340 CU per L1 block (30M cap ≈ 88k blocks); Chainstack free refuses old headers; QuickNode ≈40 credits per L1 block and is not to be spent without an explicit operator go. "Free tier" is not a capability claim — say "unverified" until the calls have run. Prefer snapshot bootstrap over re-derivation for read replicas. **Two independent failure causes (D-0124):** pruning (old receipts come back `null`) and rpckind (`quicknode` calls `debug_getRawReceipts`, which PublicNode answers with `-32601`). State whether the provider is for tip-follow or catch-up, and set the URL and its matching `--l1.rpckind` together in every environment (`SEPOLIA_L1_RPC_KIND` on the Mac, `L1_RPC_KIND` on Render) — a URL swapped without its kind stalls derivation quietly.
...
- **A new pre-redeploy gate touches four places — update them in one commit. This applies the moment a gate is *reserved*, not only when it is implemented** (D-0065): recording "F7-N is required before the announcement" in the decisions log while the four runbooks still enumerate the gate as complete leaves every operator-facing document telling the operator to proceed. That failure occurred four times in a single session before the rule was written down. The Phase 7 wipe precondition is restated in `tasks/prd-phase-7-fault-proofs.md` § Operator sequence (**binding and complete** — the authority), `README.md` § "Network reset procedure" (asserts parity with it), `.env.sepolia.example` (pointer), and `tasks/prd-l2-learning-chain.md` `:19` glossary + `:51` Phase 7 row (both self-declared **summaries**, not closed lists). Missing one leaves a reader routed to an irreversible network-wide wipe with an incomplete gate. Search by **concept** ("precondition", "redeploy", "immutables"), not by the string `FORCE_SEPOLIA_REDEPLOY` — the roadmap describes the gate without naming that command. Do **not** edit dated records (`tasks/review-*.md`, `tasks/worker-prompts/`) to match; they are history.
...
- **Toolchain locations** (all on PATH via `~/.bashrc`): Foundry `~/.foundry/bin`, Go 1.26.7 `~/go1.26/bin`, `just` + mikefarah `yq` `~/.local/bin`. Note `/usr/bin/yq` is the unrelated Python `yq`; the mikefarah `yq` (needed by `just build-superchain-go`) must precede it on PATH.

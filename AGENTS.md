...
- Prefer **small, reversible diffs**. Phases **0–6** are done (including `batcher/`, `proposer/`, `derivation/`, `blocks/`). Phase **3** Render replica is done. Do **not** execute a Sepolia redeploy, wipe replica genesis, or start Phase **3a** unless asked. Phase **7** spec is `tasks/prd-phase-7-fault-proofs.md` (learning; wipe is operator-owned; order is § Operator sequence; `rail-interface.json` stays v6 until post-wipe proxies). Phase **3b** friend runbook is `replica/FRIENDS.md` (recruiting is operator-owned). **op-reth migration** (`tasks/prd-op-reth-migration.md`): Tasks 1–7 done — chain 852 sequences on **op-reth** since 2026-09-02, promoted 2026-09-08 (D-0122; D-0120 cutover; `FORTEL2_EL=reth` in `.env.sepolia`; the op-geth datadir is kept until Task 9 (no longer a rollback obligation, D-0122)). **Task 7 closed 2026-09-12 (D-0127–D-0129):** the Render replica is `fortel2-replica-reth` (op-reth archive from snapshot 811872, `L1_RPC_FORCE=public` + `L1_RPC_KIND=standard`); public read `fortel2-replica-rpc` and SettlementOS private read point at `http://fortel2-replica-reth:10000` by env value; the geth pserv `fortel2-replica` (50 GB disk) and staging gateway `fortel2-replica-reth-rpc` are **suspended, not deleted**, until Task 9. Render private hostnames are immutable slugs — never rename Render services to move traffic (D-0128). Disk follow-up: grow `fortel2-replica-reth-data` to ≈25 GB before 2026-10-20. Do **not** flip `FORTEL2_EL` back, wipe or `debug_setHead` either datadir, resume or delete the geth pserv, re-apply the fortel2-replica Blueprint, set `karst_time`, or create `fortel2-node` unless asked. D-0125 script fixes landed (#217, D-0130): sidecar pid names are `op-reth-verifier` / `op-reth-verifier-node`; never stop the sidecar by the live name. **Live-datadir wipe/start guard landed:** `wipe_reth_datadir` and `start-op-reth-verifier.sh` refuse `$DATA_DIR/l2/op-reth` when live op-reth evidence is present (alive `op-reth` / `op-reth-node` pidfile, or a listener on `:9545`/`:9546`/`:9547`/`:9551`); a sidecar on a reth-live host must set `FORTEL2_RETH_DATADIR=$DATA_DIR/l2/spike-op-reth`. **op-challenger is intentionally STOPPED since 2026-09-13 (D-0132)** — QuickNode quota exhaustion; it had been failing every cycle since 2026-08-24 and its scan cost grows with the cumulative game count. Do not restart it as routine recovery; resolution and bond claims are handled by the hourly `resolve-games-sepolia.sh` launchd job, not the challenger. Do **not** set `SEPOLIA_CHALLENGER_GAME_WINDOW` — shrinking it saves nothing (D-0111 Finding 3, D-0132). Remaining: Task 8, Task 9. MR-3/4/5 stay parked until SOS asks.

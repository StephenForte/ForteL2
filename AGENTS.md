...
- Prefer **small, reversible diffs**. Phases **0–6** are done (including `batcher/`, `proposer/`, `derivation/`, `blocks/`). Phase **3** Render replica is done. Do **not** execute a Sepolia redeploy, wipe replica genesis, or start Phase **3a** unless asked. Phase **7** spec is `tasks/prd-phase-7-fault-proofs.md` (learning; wipe is operator-owned; order is § Operator sequence; `rail-interface.json` stays v6 until post-wipe proxies). Phase **3b** friend runbook is `replica/FRIENDS.md` (recruiting is operator-owned). **op-reth migration** (`tasks/prd-op-reth-migration.md`): Tasks 1–6 done — chain 852 sequences on **op-reth** since 2026-09-02, promoted 2026-09-08 (D-0122; D-0120 cutover; `FORTEL2_EL=reth` in `.env.sepolia`; the op-geth datadir is the kept until Task 9 (no longer a rollback obligation, D-0122)). Do **not** flip `FORTEL2_EL` back, wipe or `debug_setHead` either datadir, swap the Render replica image, set `karst_time`, or create `fortel2-node` unless asked. MR-3/4/5 stay parked until SOS asks.

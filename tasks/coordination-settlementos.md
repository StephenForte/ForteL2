...
| Phase 7 redeploy | **Notify SOS ≥1 day ahead**, and send new contract addresses once they exist — re-genesis expires every ForteL2 address they hold and breaks their live explorer address book (D-0028). Expired does not mean empty: a shifted CREATE sequence can leave an old address holding a different live contract, so they must revalidate code, type and decimals rather than assume it is dead (D-0069) |

...
| 10 | **DONE 2026-08-22 (D-0069)** — SOS-owned | SOS redeployed on 852 (full deploy, not adopt — the overlay survived while the contracts did not), re-seeded wallets, re-verified one settlement (escrow `0x1f75f257…4c96` / settlement `0x4a17351c…a5cc`, both `status=0x1`), and re-keyed the explorer address book (settlementos-explorer#58). **The old contract addresses are reassigned, not empty** — two invert token decimals (D-0069 Finding 2) | G5 |

...
| 0b | Before anyone is notified, blocking | **Prestate + game-type gate (D-0056 / D-0059 / D-0060).** Build a **Kona** prestate for chain 852 in CI (with its rollup config baked in via `KONA_CUSTOM_CONFIGS_DIR`), verify it with `cannon witness --input`, obtain `kona-host` for the mini, and teach `02-deploy-contracts-sepolia.sh` to register an **additional dispute game at type 8** carrying that prestate with `MakeRespected`. Game types 0 and 1 cannot be played at all on the pinned toolchain — op-challenger binds them to `op-program`, which is absent (D-0060). This is research plus reviewed code changes, so it sits **before** the announcement: D-0049's rule is that notice goes out only once all Phase 7 coding/config work is done | **US-071** (pre) |
...
- [ ] **Prestate built and game type 8 registered, before the notice goes out (D-0056 / D-0060, sequence step 0b):** the stateVersion-8 prestate release/type identified, the artifact in hand and its commitment confirmed with `cannon witness --input`, and `02-deploy-contracts-sepolia.sh` writing `faultGameAbsolutePrestate` into `[globalDeployOverrides]` — the script rewrites `intent.toml` every run, so this cannot be done by editing the intent file

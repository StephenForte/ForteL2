...
- **Redeploy gate** — the coordinated Sepolia (or mainnet) contract redeploy + network-wide wipe. An *event*, not a phase; it is the precondition for Phase 7 and for the mainnet pilot. It has its own prerequisites, which must be complete **before the announcement**, not merely before the redeploy — including that the deploy script pairs `ADMIN_PRIVATE_KEY` to `ADMIN_ADDRESS` before the wipe (F7-10) and refuses duplicate or absent/empty Phase 7 immutables (F7-11). `tasks/prd-phase-7-fault-proofs.md` § Operator sequence is the binding list. **Executed for Sepolia on 2026-08-22 (D-0068); steps 2–9 are complete and must not be re-run.** The gate above now describes only the *next* redeploy, the mainnet pilot.
...
- Inspect the chain with `cast` / RPC tooling in Phase 1; DIY **pipeline viewer** in Phase 1c (+ mempool polish in 1d); Phase 6 adds a simple Blockchair-style **block viewer**; defer containerized explorers (Blockscout) to a much later stage
- Deploy and interact with a simple demo dApp on the L2
- Establish a phase roadmap that explicitly sequences: bridging, pipeline viewer, Phase 2 funding gate, Sepolia cutover (2a–2d done), Render replica (Phase 3 done), friend-operated nodes (3b), per-component reimplementation (batcher → proposer → derivation + block viewer), **then** optional native Mac L1 (3a), fault proofs, decentralized sequencer
...
| **MR** | **Money rail (parallel):** SettlementOS onboards to chain **852** — see `tasks/prd-money-rail.md` + `deployments/rail-interface.json`. **SOS gate = now** (after 2c+3). Replica genesis republish still only on the redeploy gate (Phase 7 / mainnet-pilot entry) | **MR-0 done** (2026-08-04); **MR-1 done** (2026-08-13, D-0036); **MR-2 done** (2026-08-18, D-0045). MR-3/4/5 parked (D-0005) |
| **EL** | **op-geth → op-reth** (parallel; not a phase number). Spec `tasks/prd-op-reth-migration.md`. **Tasks 1–7 and 9 executed. Task 8 engineering landed; clean-room deploy unrun.** Evidence `tasks/spike-op-reth.md`, `tasks/task5-op-reth-cutover.md`. **Not** a Phase 7 wipe; do not set `karst_time`. Do not publish a new friend node on op-geth. **No geth rollback resource** after Task 9 (Render disk deleted; files return from git). **Task 9 friend-sync gate skipped, not met** (no friend has synced; two candidates expected week of 2026-09-21; operator chose to proceed). | **P:0 done** (2026-08-29) — Mini sidecar `--blocks 5` hash-matched replica; **Tasks 1–6 done — op-reth promoted 2026-09-08 (D-0122)**; Task 7 Render replica: **DONE — cut over to op-reth 2026-09-11 (D-0128)**; **Task 9 executed 2026-09-14 (fortel2-replica R-0019):** geth pserv + 50 GB disk and staging gateway deleted; public read re-verified `reth/v2.3.0-9384bc5` (recorded tip 982723 was a **stalled** head — 12.7 h replica outage from a corrupt PublicNode L1 header, recovered 2026-09-14; see PRD § Task 9). **R-0017 disk follow-up CLOSED** (reth disk 10 → 50 GB; Render has no 25 GB size). Friend Render path: fortel2-replica `RUNNING.md` § *On Render* (#58) |
| **3b** | **Friend-operated replica nodes**: recruit geographically distributed friends to run verifier nodes; onboard on **Sepolia testnet first**; proves distributed operation and shared infra ownership before any mainnet consideration | Planned — runbook `replica/FRIENDS.md`; recruiting operator-owned. Friend runbook is on **op-reth** since 2026-09-14 (`replica/FRIENDS.md`); the `fortel2-node` extraction was rejected — friends use stock `fortel2-replica` compose or **their own Render Private Service** (fortel2-replica R-0018 / #58). No friend has clean-synced yet (Task 9's skipped gate). |
...
| **7** | **Fault proofs**: run op-challenger, exercise a dispute game manually against a deliberately bad proposal. **Precondition (summary — `tasks/prd-phase-7-fault-proofs.md` § Operator sequence is binding and complete):** coordinated Sepolia redeploy with deliberately chosen immutables (fault-game clocks, `preimageOracleChallengePeriod`, `proofMaturityDelaySeconds`, `disputeGameFinalityDelaySeconds` — minutes-to-hours scale; all six knobs in one sitting), **plus the Phase 7 *code* gate closed before the announcement — the deploy script able to emit the type-8 stanza, the CI Kona-prestate workflow, `kona-host` on the mini, the deploy script pairing `ADMIN_PRIVATE_KEY` to `ADMIN_ADDRESS` before spend or wipe (F7-10), and the deploy script refusing duplicate or absent/empty immutables (F7-11) (D-0056/D-0060/D-0065, step 0b). The stateVersion-8 Kona prestate itself is built and the type-8 game registered *after* the wipe, in a second apply at step 8b, because the prestate commits to the post-wipe `rollup.json` (D-0061/D-0063)**, + completed network reset across all replica operators. **Order:** `tasks/prd-phase-7-fault-proofs.md` § Operator sequence (`rail-interface.json` stays v6 until new proxies, then v7) | **In progress — the redeploy is DONE 2026-08-22 (D-0068), steps 2–9 complete; do not re-run them.** Step 10 (SOS recovery) is done 2026-08-22 (D-0069). Step 8b (type-8 game) is done 2026-08-24 (D-0077). Step 11 (valid game watched, unattacked) is done 2026-08-24 (D-0082). Step 12 (bad proposal defeated) is done 2026-08-24 (D-0083). Step 13 (closeout note) is done 2026-08-24 (D-0085) — **demonstration complete**. Spec `tasks/prd-phase-7-fault-proofs.md` (never execute a wipe from this row) |
...
- [x] Blockscout (and any docker-compose explorer stack) is **explicitly deferred** to a **much later** stage — requires containers; revisit only after Phase 8 learning goals / non-loopback policy allows a reachable RPC, or on a different machine / native single-binary (e.g. Otterscan)
...
- [x] DIY **pipeline viewer** (ops dashboard, not a full explorer) is scoped to **Phase 1c**, after bridging so deposits/withdrawals are observable; a simple Blockchair-style **block viewer** is **Phase 6** (US-063)
...
- [x] Visual hierarchy is deliberate (brand-first hero, live status, stage flow, readable metric panels) — still named **pipeline viewer**, not explorer
...
**Out of scope for 1d:** Blockchair-style latest-blocks + block detail pages (those land in **Phase 6**), hosted/container explorers, non-loopback RPCs, custom execution clients.
...
- [x] Explicit non-goal remains: no Blockchair-like block list or block detail pages in 1d (scoped to **Phase 6** US-063); no address search / full tx explorer / Blockscout in 1d
...
### US-025: Dedicated L1 RPC via QuickNode (Phase 2d)
**Description:** As the operator, I want to swap the public Sepolia RPC for a QuickNode endpoint by changing env URLs only, so batcher/proposer polling is not rate-limited.
...
## User Stories — Phase 3 (Render L2 replica)

Phase 3 deploys a **stock** verifier on Render. Primary sync is **L1 derivation** from Sepolia (same `rollup.json` / genesis as Phase 2b) — no Mac mini port exposure required for safe/finalized heads. Sequencer P2P / tunnel is optional stretch (US-032). Containers are allowed **on Render**; the Mac mini stays native-only.

### US-030: Replica artifact pack + local verifier smoke
...
**Acceptance Criteria:**
- [ ] Out of scope for closing Phase 3 core (US-030/031); document as optional follow-up
...
**Reference shape (keep it simple):**
- List view ≈ [blockchair.com/optimism/blocks](https://blockchair.com/optimism/blocks) — table of recent blocks, **sorted by latest**
...
- FR-7: Chain inspection for Phase 1 is via documented `cast`/RPC recipes. Phase 1c adds a DIY loopback **pipeline viewer**; Phase 1d may add a small L2 mempool signal. Phase 6 adds a simple Blockchair-style **block viewer** (latest blocks → block detail; US-063). Full explorers (hosted SaaS such as Ethernal, or containerized such as Blockscout) are not required on this host and stay deferred to a much later stage
...
- FR-13: Phase 3 replica uses unmodified OP Stack EL + `op-node` (verifier). Custom derivation/sequencer work is Phase 6 (US-060–062), optionally a separate PRD; the Phase 6 block viewer is US-063
...
- No Blockscout (or other containerized explorer) on this host in Phase 1 / 1b / 1c / 1d / 2a / 4–6 — Blockscout is a **much later** stage (post–Phase 8 / when non-loopback + containers are deliberately allowed)
...
- No full address/tx search explorer and no Blockchair-style “latest blocks” pages in Phase 1c / 1d — those pages are **Phase 6** (US-063); the early deliverable is a **pipeline viewer** (+ mempool signal), not Etherscan/Blockscout feature parity
...
- Phase 6 block viewer is **not** address search, token pages, contract verification, or Blockscout — keep the Blockchair-shaped list + detail only
...
- **Resources:** without Blockscout/Postgres, RAM needs are modest (Anvil + op-node + EL + batcher + proposer + optional static pipeline / block viewers). Keep logs on disk with rotation so long runs stay manageable.
- **Pipeline viewer (Phase 1c / 1d):** prefer client-side RPC polls against existing loopback L1/L2 endpoints (same pattern as the guestbook). Do not introduce an indexer DB or container stack. Phase 1d may add `txpool_*` mempool signals; Blockchair-style block list/detail lands in **Phase 6** (US-063).
- **Block viewer (Phase 6):** same static loopback pattern as the pipeline viewer. Newest-first blocks list + expandable/navigable per-block page. No indexer, no Postgres, no Blockscout. Can share serve/config helpers with `viewer/` or live as a sibling static app — decide at implementation time.
- **Explorer deferral:** Blockscout / hosted SaaS explorers need containers and/or a non-loopback RPC. Both wait until a **much later** stage (after Phase 8 learning goals, or when US-012 explicitly allows non-loopback). Optional native single-binary explorers (e.g. Otterscan) remain an open later choice, not a Phase 6 requirement.
...
- **Phase 4–6 dependency:** the OP Stack's rollup node exposes RPCs (`optimism_syncStatus`, etc.) and the spec repo (ethereum-optimism/specs) defines batch/frame formats — reimplementation phases should target the spec, using the running stack as the reference implementation to diff against. Phase 6 derivation may be split into a separate PRD before coding starts; the Phase 6 **block viewer** (US-063) is independent of that split. All three phases run against the **pinned** 2026-07-22 Sepolia deployment (unchanged L1 contracts, no redeploy) — keeping it accumulates months of real batch history on L1, which is exactly the Phase 6 derivation test data.
...
- **Tracked advisory GO-2026-5932 (2026-07-28):** PatchHog/module scanners flag `golang.org/x/crypto` (indirect in `batcher/go.mod` + `proposer/go.mod`, currently `v0.55.0`) because `x/crypto/openpgp` is unmaintained and unsafe by design — **no fixed version exists** for that subtree ([pkg.go.dev/vuln/GO-2026-5932](https://pkg.go.dev/vuln/GO-2026-5932)). ForteL2 does not import `openpgp` (only other `x/crypto` packages via `go-ethereum`); `govulncheck ./…` is clean in both modules. **Action:** keep tracking until upstream retires the package from the module surface or `go-ethereum` drops the dependency path; on each Go dep bump re-run `govulncheck`; never add `openpgp` imports — use ProtonMail’s fork if OpenPGP interoperability is ever needed. Operator-facing note: `README.md` → Tracked dependency advisories.
...
- **Sepolia deployment lifecycle (2026-07-22):** the Phase 2b deploy (current `deployments/sepolia/deployments.json`) is **pinned through Phases 4–6** — no longer disposable. Next redeploy is the **redeploy gate** (Phase 7 / mainnet-pilot entry): choose all six new immutables in one sitting (fault-game clocks, `preimageOracleChallengePeriod`, `proofMaturityDelaySeconds`, `disputeGameFinalityDelaySeconds`) sized minutes-to-hours for realistic dispute games, then run the coordinated network reset (announce → publish artifacts → all operators wipe → all restart → cross-check hashes). A forgotten parameter — including the preimage-oracle period, whose stock 86400 makes a 2h max clock illegal — means a second redeploy and a second network-wide wipe.

...
| **EL** | **op-geth → op-reth** (parallel; not a phase number). Spec `tasks/prd-op-reth-migration.md`. **Tasks 1–4 done** (#176/#178/#184/#189; D-0109/D-0110/D-0114/D-0116): pin, selector, candidate derived to safe head with parity, fault-proof/SafeDB/withdrawal-prove proven. Task 5 Phase A = selector-gated scripts (default geth; merging flips nothing). Phase B (live cutover) is operator-owned; gate = D-0116 finalize tail. Evidence `tasks/spike-op-reth.md`, `tasks/task5-op-reth-cutover.md`. **Not** a Phase 7 wipe; do not set `karst_time`. Do not publish a new friend node on op-geth once this track is moving | **P:0 done** (2026-08-29) — Mini sidecar `--blocks 5` hash-matched replica; **Tasks 1–6 done — op-reth promoted 2026-09-08 (D-0122)**: chain 852 sequencing on op-reth, all §8 end-to-end checks pass, ~134 h / 8 cycles observed; Task 7 Render replica: Phase A merged; snapshot-bootstrap dry-run PASSED (D-0125); restore gated on the archive-vs-`--full` storage decision (D-0125; irreversible datadir, pending a consumer sweep) — not on an L1 provider |
| **3b** | **Friend-operated replica nodes**: recruit geographically distributed friends to run verifier nodes; onboard on **Sepolia testnet first**; proves distributed operation and shared infra ownership before any mainnet consideration | Planned — runbook `replica/FRIENDS.md`; recruiting operator-owned. EL for new friends is the migration PRD Task 8 (`fortel2-node`); current runbook stays op-geth until then |

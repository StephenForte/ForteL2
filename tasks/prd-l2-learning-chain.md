...
| **7** | **Fault proofs**: run op-challenger, exercise a dispute game manually against a deliberately bad proposal. **Precondition (summary — `tasks/prd-phase-7-fault-proofs.md` § Operator sequence is binding and complete):** coordinated Sepolia redeploy with deliberately chosen immutables (fault-game clocks, `preimageOracleChallengePeriod`, `proofMaturityDelaySeconds`, `disputeGameFinalityDelaySeconds` — minutes-to-hours scale; all six knobs in one sitting), **plus a resolved and pinned stateVersion-8 Kona absolute prestate and a registered game type 8 (D-0056/D-0060, step 0b — before the announcement)**, + completed network reset across all replica operators. **Order:** `tasks/prd-phase-7-fault-proofs.md` § Operator sequence (`rail-interface.json` stays v6 until new proxies, then v7) | Planned — spec `tasks/prd-phase-7-fault-proofs.md` (do not execute the wipe from this row) |

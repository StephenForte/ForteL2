...
| **EL** | **op-geth → op-reth** (parallel; not a phase number). Spec `tasks/prd-op-reth-migration.md`. Evidence `tasks/spike-op-reth.md`. Sequencer cutover is PRD Task 5 and operator-owned. **Not** a Phase 7 wipe; do not set `karst_time`. Do not publish a new friend node on op-geth once this track is moving | **P:0 done** (2026-08-29) — Mini sidecar `--blocks 5` hash-matched replica; later tasks unstarted |

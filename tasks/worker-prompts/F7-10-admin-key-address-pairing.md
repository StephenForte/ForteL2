...
Host: any. **No `.env.sepolia`, no Sepolia keys, no `op-deployer` run, no network, no spend.** Nothing in this task signs, deploys, or wipes anything. You are editing a script and asserting on its behaviour with throwaway keys.
...
**Use a throwaway keypair** — `cast wallet new`, or a hard-coded non-Foundry test key. Do **not** use a Foundry/Anvil default: `refuse_foundry_defaults_unless_local_l2` fires first on any non-901 chain and you will be testing the wrong refusal.

...
9. Prints the tx hash and tells the operator to run `go run ./cmd/inspect-game -index <gameCount-1>` (existing tool, unmodified) to retrieve the resulting game proxy address and confirm the on-chain `rootClaim` matches your printed corrupted value — do not reimplement receipt-polling/game-lookup in this tool; that already exists.

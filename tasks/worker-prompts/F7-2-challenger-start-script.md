...
- `require_min_balance_eth "$CHALLENGER_ADDRESS" "${SEPOLIA_CHALLENGER_MIN_ETH:-0.15}" "CHALLENGER"` — the challenger posts bonds and moves, so it needs gas like the proposer does.
...
- `wait_for_rpc` on `$L1_RPC_URL` and `$L2_NODE_RPC_URL` before starting.
...
- The script must fail closed: missing key, missing trace type, missing prestate, unfunded challenger, or failed preflight all exit non-zero **before** `start_bg`.

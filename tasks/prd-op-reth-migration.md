...
| 4 | Does Render public RPC need archive, or is `--full` enough? | **Partly answered (Mini dry-run, D-0125):** `--full` returns **null** for historical `eth_getTransactionReceipt` (block 5) while the current geth `--gcmode=full` replica and the archive sequencer serve it — a regression. `--full` is enough only if no replica consumer needs historical receipts/logs/tx-by-hash; otherwise the reth replica needs an **archive** profile. Decision gates the restore (a reth datadir cannot be un-pruned). Pruning boundary unswept. |

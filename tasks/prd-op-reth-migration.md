...
**Status:** In execution — Tasks 1–6 done, op-reth promoted (D-0122); **Task 7 Phase A merged; snapshot bootstrap chosen (D-0124), Mini dry-run PASSED (D-0125), archive profile chosen (D-0126), and the Render restore EXECUTED 2026-09-10 (D-0127): `fortel2-replica-reth` runs op-reth in archive mode from snapshot 811872 and is at tip on PublicNode**. Phase B (parity, gateway, restart, Q5 disk/RSS) is next, then Phase C (repoint); Tasks 8–9 unstarted  
**Date:** 2026-08-29  
...
| 5 | Disk/RAM of a clean 852 op-reth sync on Render under current RPC load? | **Open — Phase B measures.** Data so far (D-0127): archive datadir ≈1.2 GB at ~823k blocks on the Mac; Render disk is 10 GB, plan Standard (1c-2g). Need Render-side growth per day and op-reth/op-node RSS at tip. |
...
- **Task 7 (Render replica):** Phase A done (image/service, fortel2-replica #44/#45). From-genesis catch-up abandoned (D-0123); snapshot bootstrap (D-0124, #46), Mini dry-run PASSED (D-0125), archive profile chosen (D-0126), archive mode + persisted-prune fix landed (#47, #48 / R-0015), and the **Render restore EXECUTED 2026-09-10 (D-0127)**: `fortel2-replica-reth` is archive, resumed from safe origin 11676027, caught up 811875→822825 in ≈31 min, FORCE unset and verified. Not yet verified on Render: parity vs sequencer, gateway checks, restart behaviour, Q5 disk/RSS (Phase B).

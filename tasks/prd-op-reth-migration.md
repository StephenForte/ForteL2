...
**Status:** Tasks 1–7 and Task 9 executed. Chain 852 sequences on op-reth since 2026-09-02 (promoted D-0122). **Task 7 CLOSED (D-0129, 2026-09-12):** Render replica `fortel2-replica-reth` is op-reth archive and serves public read + SettlementOS private read. **Task 8 engineering landed** (R-0018, #53/#55/#56, #58 / R-0020); **clean-room deploy still unrun** (no docker host in the loop) — do not read Task 8 as verified. **Task 9 executed 2026-09-14 (fortel2-replica R-0019, #56/#57):** geth EL files deleted from that repo; operator deleted the geth pserv `fortel2-replica` (`srv-d9fsgi3rjlhs73ceh6tg`, 50 GB disk `dsk-d9g1mmsm0tmc73degtrg`) and staging gateway `fortel2-replica-reth-rpc` (`srv-dagr42tbedkc73c5mp80`) that had been suspended since 2026-09-12. Re-verified after: public read `reth/v2.3.0-9384bc5`; SettlementOS `/api/networks` 200. **That check was wrong about liveness** — the recorded `tip 982723` was a *stalled* head and the Render replica was down 12.7 h (§ Task 9, *Post-delete verification was wrong*). **There is no geth rollback resource** — files return from git; the Render disk does not; recovery is a new disk and a full resync. **Task 9's friend-sync gate was skipped, not met:** Instructions required at least one friend clean sync; none has happened; two candidates expected the week of 2026-09-21; the operator chose to proceed. **R-0017 disk follow-up CLOSED:** reth disk resized 10 → 50 GB the same day (Render offers fixed sizes; 25 is not one). Learning oracles (`derivation-check.sh`, `sequencer-stub-demo.sh`) still start isolated `op-geth` — keep-on-geth vs follow-up is an operator decision, not a silent live-path leftover.
...
- Re-verified after: public read `https://fortel2-replica-rpc.onrender.com` answers `web3_clientVersion = reth/v2.3.0-9384bc5`; SettlementOS `/api/networks` 200. The tip reading taken at the same time (982723) **was not a liveness check** — see below.
...
**Post-delete verification was wrong, and the replica was down 12.7 h (2026-09-14).** The closeout recorded `tip 982723` as evidence the reth path was healthy. It was a bare `eth_blockNumber` with no staleness test, and 982723 was the block the Render replica had **stopped** at. Found while reviewing this PR at 20:52 PDT: the replica had not derived since 08:12:14 PDT while the sequencer ran normally (`safe_l2` 190 s old). Block 982723 hashed identically on both sides — stalled, not forked.

**Cause: a corrupt header on PublicNode, not anything Task 9 did.** op-node requested Sepolia L1 header `11703708`; PublicNode returned `11703707`'s header. op-node correctly refuses a header whose ID does not match the request, and retried every ~10 s for 12.7 h. Reproduced directly: 11703706 ok, 11703707 ok, **11703708 → returns 11703707**, 11703709 ok — on both `ethereum-sepolia-rpc.publicnode.com` and `ethereum-sepolia.publicnode.com`. The same host serves 162 receipts at that height, so it is their by-number header index, not missing data. The service ran continuously throughout (same instance; last deploy 2026-09-10), so the 08:12 proximity to the Task 9 deletes is coincidence, not cause. Restarts do not help: a fresh process walked 11703588 → 11703707 cleanly and wedged again at 11703708.

**Recovery (2026-09-14 21:38 PDT).** Moved L1 off PublicNode via `L1_RPC_FORCE=metered`. The first attempt failed closed: the stored `L1_RPC_URL` was a free Chainstack endpoint that 403s the L1 genesis check (*"Archive, Debug and Trace requests are not available on your current plan"*), so op-node exited and Render rolled back to the public-mode image. With `L1_RPC_URL` on QuickNode it crossed 11703708 at once and caught up ~25,000 L2 blocks in ~41 min; block 1008000 hash-matched the sequencer at 22:22 PDT. **Resolved (D-0136): `L1_RPC_FORCE` stays `metered`.** It was briefly returned to `public` on 2026-09-15 and the replica immediately fell behind at 0.35 L1 blocks/min against Sepolia's ~5/min — tip frozen 3 h+, gap widening, no errors. Restored to `metered` the same morning (~$0.19/day). Do not treat `public` as the steady state for this service.

**Two things this leaves.** `L1_RPC_FORCE=public` trades liveness for QuickNode credits, and that trade cost 12.7 h of replica downtime on a path whose rollback had been deleted the same morning — the steady-state endpoint choice deserves a decision record. And nothing alerted: `scripts/alert-watch.sh` covers the Mac stack, so a Render replica frozen for 12.7 h while answering RPC normally is invisible to it.
...
- [x] Public read gateways still reject writes. (re-verified 2026-09-14 after Task 9 deletes. Write-rejection only — the tip reading taken alongside it was a stalled head; see § Task 9.)
